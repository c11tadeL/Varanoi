...
namespace Program
{
    public sealed partial class Form1 : Form
    {
        ...
        private void DrawMultiThread()
        {
            ...
            for (int t = 0; t < Environment.ProcessorCount; t++)
            {
                bitmaps.Add(new Bitmap(_bitmap.Width, _bitmap.Height));
            }
            ...
            for (int t = 0; t < Environment.ProcessorCount; t++)
            {
                ...
                Bitmap localBitmap = bitmaps[t];

                tasks.Add(Task.Run(() =>
                {
                    using (Graphics graphics = Graphics.FromImage(localBitmap))
                    {
                        for (int x = startX; x < endX; x++)
                        {
                            for (int y = 0; y < segmentHeight; y++)
            ...
            Task.WaitAll(tasks.ToArray());
            using (Graphics mainGraphics = Graphics.FromImage(_bitmap))
            {
                foreach (Bitmap localBitmap in bitmaps)
                {
                    mainGraphics.DrawImage(localBitmap, 0, 0);
                }
            }
        }
